...
const Distributor = () => {
    ...
    const getMarkertsApi = async () => {
        const res = await axios.get('http://192.168.157.27/api/distributor/list');
        setUbications(res.data.data);
    }

    const getApiGeode = async () => {
        if (latitude && longitude) {
            const res = await axios.get(`http://192.168.157.27/api/distributor/find/point/close?longitude=${longitude}&latitude=${latitude}`)
