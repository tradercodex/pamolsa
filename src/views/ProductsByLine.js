...
const ProductByLine = ({ match, location }) => {
    ...
    const apiGetTypes = async () => {
        const response = await Axios.get(`http://192.168.157.27/api/product/types/list?line_id=${line}`);
        setTypes(response.data.data)
    }

    const apiGetBusiness = async () => {
        const response = await Axios.get(`http://192.168.157.27/api/product/business/list?line_id=${line}`);
        setBusiness(response.data.data)
    }

    const apiGetMaterial = async () => {
        const response = await Axios.get(`http://192.168.157.27/api/product/material/list?line_id=${line}`);
